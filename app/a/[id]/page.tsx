...
/* ── CSS matching index.html design system exactly ── */
const CSS = `
...
.sa .text-muted { color: var(--text-muted) !important; }
...
/* ── Score Card (matches index.html .score-card exactly) ── */
.score-card {
  max-width: 400px;
  margin: 0 auto 48px;
...
/* ── Plan Headers (matches index.html .plan-header) ── */
.plan-header {
  text-align: center;
  padding: 32px 0 28px;
}
...
.plan-divider.detective {
  background: linear-gradient(90deg, transparent, #3B82F6, transparent);
}
.plan-divider.obsessive {
...
/* ── Stats Grid (matches index.html .stats-grid) ── */
.stats-grid {
  display: grid;
...
/* ── Fun Fact / AI Insight (matches .fun-fact) ── */
.fun-fact {
...
/* ── Example Card / Dynamics (matches .example-card) ── */
.example-card {
  background: var(--bg-card);
  border-radius: var(--radius);
  padding: 32px;
...
/* ── Premium Feature Cards (matches .premium-feature-card) ── */
      ...
      <>
        ...
        <div className="sa">
          ...
          <div style={{ textAlign: 'center', marginBottom: 24 }}>
            ...
            <p className="text-muted" style={{ marginBottom: 24 }}>Este enlace no existe o ha expirado.</p>
          </div>
          <div className="sa-error">
            <strong>Debug:</strong><br/>ID: {id}<br/>Error: {error?.message || 'none'}<br/>Code: {error?.code || 'none'}
          </div>
              ...
              return (
                <div key={sec.key} className="premium-feature-card">
                  ...
                  <div className="pf-body">
                    {/* Chart image for timeline */}
                    {sec.key === 'timeline' && chartImages.timeline && (
                      <img
                        className="chart-img"
                        src={chartImages.timeline}
                        alt="Línea de Vida de la Relación"
                      />
                    )}
                    {body}
                  </div>
